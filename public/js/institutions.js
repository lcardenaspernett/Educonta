// ===================================
// EDUCONTA - Gestión de Instituciones
// ===================================
...
async function editInstitution(institutionId) {
    console.log('🔧 INICIANDO EDICIÓN DE INSTITUCIÓN:', institutionId);
...
async function handleInstitutionFormSubmit(e) {
    e.preventDefault();
    console.log('🚀 ENVIANDO FORMULARIO DE INSTITUCIÓN');

    const token = localStorage.getItem('token');
    console.log('🔧 TOKEN EN FORM SUBMIT:', token ? 'EXISTS' : 'MISSING');

    if (!token) {
        console.log('❌ NO TOKEN FOUND IN FORM SUBMIT');
    ...
    // CONSTRUCCIÓN MEJORADA DEL FORM DATA - SIN NULLS
    const formData = {};

    // Campos requeridos
    formData.name = document.getElementById('institution-name').value.trim();
    formData.nit = document.getElementById('institution-nit').value.trim();
    formData.city = document.getElementById('institution-city').value.trim();
    formData.department = document.getElementById('institution-department').value.trim();
    formData.educationLevel = document.getElementById('institution-education-level').value;
    formData.address = document.getElementById('institution-address').value.trim();

    // Campos opcionales - SOLO SI TIENEN VALOR
    const phone = document.getElementById('institution-phone').value.trim();
    if (phone) {
        formData.phone = phone;
        console.log('✅ Agregando phone:', phone);
    }

    const email = document.getElementById('institution-email').value.trim();
    if (email) {
    ...
    if (website) {
        ...
        console.log('✅ Agregando website:', website);
    }

    const description = document.getElementById('institution-description').value.trim();
    if (description) {
        formData.description = description;
        console.log('✅ Agregando description:', description);
    }

    console.log('🔧 FORM DATA FINAL (SIN NULLS):', JSON.stringify(formData, null, 2));
